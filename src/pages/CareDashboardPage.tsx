...
import { UI, PIE_COLORS, ASSIGNEE_COLORS, ASSIGNEE_OTHERS_COLOR, MAX_ASSIGNEE_SERIES, heatBg } from "@/lib/theme";
import {
  parseCreated,
  ym,
  teamSizeForMonth,
...
import { StatCard, KpiPreviousPeriod } from "@/components/StatCard";
...
// Wrapper sobre <StatCard/> (src/components/StatCard.tsx) que preserva la
// firma posicional usada en el resto de este archivo, para no tener que
// tocar cada call site al extraer el componente compartido.
function kpiCard(
  title: string,
  value: any,
...
// backend propio para guardarlo). Tipo `OrgMapping` importado de
...
export default function CareDashboardPage() {
  ...
  // rows/janisRows/error/autoRange/fromMonth/toMonth y el parseo de CSV
  // (onFile/onJanisFile) viven en el contexto compartido (src/context/
  // DashboardDataContext.tsx) para que /agentPerformance reutilice los
  // mismos datos sin re-subir archivos ni duplicar el parser.
  const { rows, janisRows, error, setError, autoRange, fromMonth, setFromMonth, toMonth, setToMonth, onFile, onJanisFile, clearAll: clearSharedData } =
    useDashboardData();
  ...
  return (
    <div className={`min-h-screen ${UI.pageBg} p-4 md:p-8`}>
      <div className="mx-auto max-w-7xl" ref={reportRef}>
        <ViewNav />
        {rows.length === 0 ? <UploadPrompt /> : null}
        ...
        {/* Heatmaps */}
        <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2">
          <HourHeatmap data={series.hourHeatMap.data} max={series.hourHeatMap.max} />
